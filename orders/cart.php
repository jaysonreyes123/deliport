            ...
            <div id="cart-list">
                ...
                ?>
                    ...
                    <div class="col-12 border p-0">
                        ...
                        $quantity = 0;
                        $pid = 0;
                        $products = $conn->query("SELECT c.*, p.name as `name`,p.image_path FROM `cart_list` c inner join product_list p on c.product_id = p.id where c.client_id = '{$_settings->userdata('id')}' and p.vendor_id = '{$vrow['id']}' order by p.name asc");
