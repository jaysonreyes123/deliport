...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT o.*,v.shop_name,v.code as vcode from `order_list_vendor` o inner join vendor_list v on o.vendor_id = v.id where o.id = '{$_GET['id']}' and is_supplier = 1 ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
    ...
    <?php 
        $order_id = array();
        $gtotal = 0;
        $products = $conn->query("SELECT o.*, p.name as `name`, o.price,p.image_path FROM `order_items_vendor` o inner join product_list_vendor p on o.product_id = p.id where o.order_id='{$id}' order by p.name asc");
...
<script>
    ...
    function submit_feedback($id){
        ...
        start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=submit_feedback_vendor",
			method:"POST",
			data:form,
            dataType:"json",
            contentType:false,
    ...
    function cancel_order($id){
        start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=cancel_order_vendor",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
