    ...
    <div class="row">
        <div class="col-md-4">
            <div class="card card-outline rounded-0 card-primary shadow">
                <div class="card-body">
                    <select name="shop_type" id="shop_type" class="form-control mb-2">
                        <?php
                            $ctr = 0;
                            $shop_type_ = $conn->query("SELECT * from shop_type_list where `status` = 1 and delete_flag = 0 ");
                            while($row = $shop_type_->fetch_assoc()):
                                if($ctr == 0){
                                    $shop_type =  $_GET["shop_type"] ?? $row["id"]; 
                                }
                                $ctr++;
                        ?>
                        <option <?=$shop_type == $row["id"] ? "selected" : ""?> value="<?=$row["id"]?>"><?=$row["name"]?></option>
                        <?php endwhile; ?>
                    </select>
                    <div class="list-group">
                        ...
                        $categories = $conn->query("SELECT * FROM `category_list`  where shop_type='{$shop_type}' and delete_flag = 0 and status = 1 order by `name` asc ");
                ...
                <div class="card-body">
                    <div class="container-fluid">
                        ...
                        <div class="row" id="product_list">
                            ...
                            $products = $conn->query("SELECT p.*, v.shop_name as vendor, c.name as `category` FROM `product_list_vendor` p inner join vendor_list v on p.vendor_id = v.id inner join category_list c on p.category_id = c.id where p.delete_flag = 0 and c.shop_type = '{$shop_type}' and p.`status` =1 {$swhere} order by RAND()");
                            while($row = $products->fetch_assoc()):
                            ?>
                            <div class="col-lg-4 col-md-6 col-sm-12 product-item">
                                <a href="./?page=takeorder/view_product&id=<?= $row['id'] ?>" class="card shadow rounded-0 text-reset text-decoration-none">
                                <div class="product-img-holder position-relative">
                                    <img src="<?= validate_image($row['image_path']) ?>" alt="Product-image" class="img-top product-img bg-gradient-green">
                                </div>
...
<script>
    $(function(){
        ...
        $('.cat_item').change(function(){
            ...
            location.href="./?page=takeorder&shop_type="+$("#shop_type").val()+"&cids="+(ids.join(","))
        })

        $("#shop_type").change(function(){
            location.href="./?page=takeorder&shop_type="+$("#shop_type").val();
        })
        ...
        $('#search-frm').submit(function(e){
            ...
            location.href="./?page=takeorder&"+q;
