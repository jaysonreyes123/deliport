...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT  p.*, v.shop_name as vendor, c.name as `category` FROM `product_list_vendor` p inner join vendor_list v on p.vendor_id = v.id inner join category_list c on p.category_id = c.id where p.delete_flag = 0 and p.id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
        foreach($qry->fetch_assoc() as $k => $v){
            $$k=$v;
        }
    }else{
        echo "<script> alert('Unkown Product ID.'); location.replace('./?page=takeorder') </script>";
        exit;
    }
}else{
    echo "<script> alert('Product ID is required.'); location.replace('./?page=takeorder') </script>";
    exit;
}
?>
<style>
        ...
        <div class="card-body">
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4">
                        <?php
                            $get_ratings = $conn->query("SELECT a.*,b.firstname,b.lastname from order_items_vendor a left join vendor_list b on a.client_id = b.id where  product_id = '{$_GET['id']}' ");
                            $total_rating = 0;
                            $ctr =0;
                            $list_item = "";
...
var rating = $(".rate_all").data("value");
    ...
    function add_to_cart(){
        ...
        start_loader()
        $.ajax({
            url:_base_url_+'classes/Master.php?f=add_to_cart_vendor',
            method:'POST',
