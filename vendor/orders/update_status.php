...
if($_settings->userdata('is_supplier') == 1){
	...
	$update_status = "update_status_vendor";
...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `$orderlist` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<input type="hidden" value="<?=$update_status?>" id="update_status__">
<script>
    $(function(){
		var update_status__ = $("#update_status__").val();
        $('#uni_modal_second #update_status').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f="+update_status__,
