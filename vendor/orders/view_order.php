...
$orderitem = "";
$vendor = "";
if($_settings->userdata('is_supplier') == 1){
    $orderlist = "order_list_vendor";
    $productlist = "product_list_vendor";
...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.lastname, ', ',c.firstname,' ',COALESCE(c.middlename,'')) as client from `$orderlist` o inner join `$vendor` c on o.client_id = c.id where o.id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	<div class="row">
        ...
        <div class="col-3 border bg-gradient-primary"><span class="">Rating</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($rate) ? $rate : '' ?></span></div>
        
        <div class="col-3 border bg-gradient-primary"><span class="">Feedback</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($feedback) ? $feedback : '' ?></span></div>
    </div>
    <div class="clear-fix mb-2"></div>
    <div id="order-list" class="row">
    <?php 
        $gtotal = 0;
        $products = $conn->query("SELECT o.*, p.name as `name`, o.price,p.image_path FROM `$orderitem` o inner join `$productlist` p on o.product_id = p.id where o.order_id='{$id}' order by p.name asc");
