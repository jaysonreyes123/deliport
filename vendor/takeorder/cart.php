...
<div class="content py-3">
            ...
            <div id="cart-list">
                ...
                ?>
                    ...
                    <div class="col-12 border p-0">
                        ...
                        $quantity = 0;
                        $pid = 0;
                        $products = $conn->query("SELECT c.*, p.name as `name`,p.image_path FROM `cart_list_vendor` c inner join product_list_vendor p on c.product_id = p.id where c.client_id = '{$_settings->userdata('id')}' and p.vendor_id = '{$vrow['id']}' order by p.name asc");
                        ...
                        <div class="d-flex align-items-center border p-2">
                            ...
                            <div class="col-auto flex-shrink-1 flex-grow-1">
                                <h4><b><?= $prow['name'] ?></b></h4>
                                <?php $parse_ = explode("_",$prow["type"]);
                                    $type = $parse_[0];
                                    $size = $parse_[1];
                                ?>
                                <div class="d-flex">
                                    <div class="col-auto px-0"><small class="text-muted">Type: </small></div>
                                    <div class="col-auto px-0 flex-shrink-1 flex-grow-1"><p class="m-0 pl-3"><small><?= $type ?? "" ?></small></p></div>
                                </div>

                                <div class="d-flex">
                                    <div class="col-auto px-0"><small class="text-muted">Size: </small></div>
                                    <div class="col-auto px-0 flex-shrink-1 flex-grow-1"><p class="m-0 pl-3"><small><?= strtoupper($size) ?? "" ?></small></p></div>
                                </div>
    ...
    <div class="text-right">
        <!-- <a href="./?page=takeorder/checkout" class="btn btn-flat btn-primary btn-sm"><i class="fa fa-money-bill-wave"></i> Checkout</a> -->
        <div class="btn-group">
            ...
            <div class="dropdown-menu dropdown-menu-right">
                <a class="dropdown-item" href="./?page=takeorder/checkout&method=order">Order</a>
                <a class="dropdown-item" href="./?page=takeorder/checkout&method=pre-order"> Pre order</a>
            </div>
        </div>
    </div>
</div>
<script>
    $(function(){
        $('.plus-qty').click(function(){
            ...
            start_loader()
            $.ajax({
                url:_base_url_+'classes/Master.php?f=update_cart_qty_vendor',
                method:'POST',
    ...
    function delete_cart($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=delete_cart_vendor",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
